...
export default class FishWatcher {
  constructor(weatherService) {
    //JL: Pass in weather service instance from ViewModel
    console.log("Constructing watcher")
...
//JL: Don't need fisheyes
  ...
  updateRangesForFish(fish, baseTime) {
    ...
    var weatherIter = this.weatherService.findWeatherPattern(
      startOfWindow,
    ...
    this.weatherService.finishedWithIter();
    ...
    while (lastRangeSpansPeriods) {
      ...
      var iter = this.weatherService.findWeatherPattern(
    ...
    if (_(fish.predators).size() > 0) {
      ...
      _(fish.predators).chain().keys().each((predId) => {
        ...
        if (_iterItem.done) {
          ...
          iter = this.weatherService.findWeatherPattern(
