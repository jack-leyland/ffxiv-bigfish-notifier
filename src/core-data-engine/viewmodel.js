...
import { Subject, BehaviorSubject, merge, interval, buffer, debounceTime, map, filter, skip, timestamp } from "rxjs"
import LocalizationHelper from "./localization.js";
...
class FishEntry {
  ...
  update(earthTime, full = false) {
    ...
    //JL: Anything this class is asking from the ViewModel is not relevant?
...
export class ViewModel {
  constructor() {
    // JL: This is our new main class. We are going to string all the UI functionality
    // out of this, leaving only what's needed for the actual raw fishEntry Data.
    console.log("contructing view model")
  ...
  initializeDisplay() {
    ...
    //JL: This stuff might be important?
    ...
    const language$ = this.localizationHelper.languageChanged.pipe(
      skip(1),
  ...
  applySettings(settings) {
    //JL: WTake all the JQuery stuff out.
    ...
    if (settings.filters.patch) {
      ...
      if (settings.latestPatch !== LATEST_PATCH) {
        ...
        //     this.settings.filters.patch.add(Number(LATEST_PATCH));

        //     // FIXME: Duplicate code!
        //     // $('#filterPatch .button').removeClass('active');
        //     for (let includedPatch of this.settings.filters.patch) {
        ...
        //     // Notify others about the change.
        //     this.filterPatchSubject.next(ViewModel.settings.filters.patch);
        //     this.saveSettings();
